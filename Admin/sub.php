<?php
/***** 1) الجلسة والتحقق *****/
session_start();
if (!isset($_SESSION['stable_id'])) { header('Location: stable_login.php'); exit; }
$STABLE_ID = (int) $_SESSION['stable_id'];

/***** 2) الاتصال بقاعدة البيانات عبر dbcon.php مع كتم أي مخرجات *****/
ob_start();
require_once __DIR__ . '/dbcon.php'; // يوفر $conn (mysqli)
ob_end_clean();
if (!$conn) { die('DB connection error'); }

// لا نلمس أي شيء غير جلب مسار الصورة
if (session_status() === PHP_SESSION_NONE) session_start();

$__profile_src__ = 'logo1.png'; // نفس الصورة الحالية كافتراضي

if (!empty($_SESSION['stable_id'])) {
    // استخدمي اتصالك الحالي إن وُجد، وإلا نحاول تضمين dbcon.php بدون لمس شيء
    if (!isset($conn)) {
        $dbconPath = __DIR__ . '/dbcon.php';
        if (file_exists($dbconPath)) {
            require_once $dbconPath; // يُفترض أنه يعرّف $conn (mysqli)
        }
    }

    if (isset($conn) && $conn) {
        $sid = (int)$_SESSION['stable_id'];
        if ($stmt = mysqli_prepare($conn, "SELECT Profile_Image FROM Stable WHERE Stable_ID=?")) {
            mysqli_stmt_bind_param($stmt, 'i', $sid);
            mysqli_stmt_execute($stmt);
            $res = mysqli_stmt_get_result($stmt);
            if ($row = mysqli_fetch_assoc($res)) {
                if (!empty($row['Profile_Image'])) {
                    $__profile_src__ = $row['Profile_Image']; // مثال: uploads/stables/stable_3.jpg
                }
            }
            if ($res) mysqli_free_result($res);
            mysqli_stmt_close($stmt);
        }
    }
}
/***** [اختياري للتشخيص] فعّل الأسطر التالية مؤقتًا لو استمر 500 */
// ini_set('display_errors', 1);
// error_reporting(E_ALL);
// mysqli_report(MYSQLI_REPORT_ERROR | MYSQLI_REPORT_STRICT);

/***** 3) دوال مساعدة *****/
function fetch_all($conn, $sql, $types='', ...$params){
  $stmt = mysqli_prepare($conn, $sql);
  if ($types!=='') mysqli_stmt_bind_param($stmt, $types, ...$params);
  mysqli_stmt_execute($stmt);
  $res = mysqli_stmt_get_result($stmt);
  $rows = [];
  while($r = mysqli_fetch_assoc($res)) $rows[] = $r;
  mysqli_free_result($res);
  mysqli_stmt_close($stmt);
  return $rows;
}
function fetch_one($conn, $sql, $types='', ...$params){
  $stmt = mysqli_prepare($conn, $sql);
  if ($types!=='') mysqli_stmt_bind_param($stmt, $types, ...$params);
  mysqli_stmt_execute($stmt);
  $res = mysqli_stmt_get_result($stmt);
  $row = mysqli_fetch_assoc($res) ?: [];
  mysqli_free_result($res);
  mysqli_stmt_close($stmt);
  return $row;
}
function h($v){ return htmlspecialchars((string)$v, ENT_QUOTES, 'UTF-8'); }

/***** 4) إضافة اشتراك (AUTO_INCREMENT + Stable_ID الحالي) *****/
$flash = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_sub'])) {
  $name   = trim($_POST['sub_name'] ?? '');
  $limit  = trim($_POST['session_limit'] ?? '');
  $class  = trim($_POST['class_type'] ?? '');
  $days   = trim($_POST['duration_days'] ?? '');
  $price  = trim($_POST['price'] ?? '');

  if ($name==='' || $limit==='' || !ctype_digit($limit) || $class==='' || $days==='' || !ctype_digit($days) || $price==='' || !is_numeric($price)) {
    $flash = 'Please fill all fields correctly.';
  } else {
    // منع تكرار الاسم داخل نفس الإسطبل (اختياري)
    $dup = fetch_one($conn,"
      SELECT COUNT(*) AS c FROM Subscription
      WHERE Stable_ID = ? AND Sub_name = ?
    ", 'is', $STABLE_ID, $name);

    if ((int)($dup['c'] ?? 0) > 0) {
      $flash = 'Subscription name already exists in your stable.';
    } else {
      // تجهيز القيم بالأنواع الصحيحة
      $limitInt  = (int)$limit;          // SessionLimit (INT)
      $daysInt   = (int)$days;           // DurationDays (INT)
      $priceF    = (float)$price;        // Price (DECIMAL/DOUBLE)
      $stableInt = (int)$STABLE_ID;      // Stable_ID (INT)

      // ملاحظة: حسب جدولك الأعمدة هي Sub_name, SessionLimit, Class_Type, DurationDays, Price, Stable_ID
      $stmt = mysqli_prepare($conn, "
        INSERT INTO Subscription (Sub_name, SessionLimit, Class_Type, DurationDays, Price, Stable_ID)
        VALUES (?,?,?,?,?,?)
      ");
      // الأنواع بلا مسافات: s (name), i (limit), s (class), i (days), d (price), i (stable)
      mysqli_stmt_bind_param($stmt, 'sisidi',
        $name, $limitInt, $class, $daysInt, $priceF, $stableInt
      );

      $ok = mysqli_stmt_execute($stmt);
      mysqli_stmt_close($stmt);

      if ($ok) { header('Location: sub.php?added=1'); exit; }
      else { $flash = 'Insert failed.'; }
    }
  }
}

/***** 5) حذف اشتراك (ممنوع إذا عليه حجوزات) *****/
if (isset($_GET['delete']) && ctype_digit($_GET['delete'])) {
  $sid = (int) $_GET['delete'];
  // تأكد أنه يخص هذا الإسطبل
  $owner = fetch_one($conn, "SELECT Stable_ID FROM Subscription WHERE Subscription_ID = ?", 'i', $sid);
  if (!$owner || (int)$owner['Stable_ID'] !== $STABLE_ID) {
    $flash = 'You can only delete subscriptions in your stable.';
  } else {
    // ممنوع حذف إذا عليه حجوزات
    $inuse = fetch_one($conn,"
      SELECT COUNT(*) AS c
      FROM Booking b
      WHERE b.Subscription_ID = ?
    ", 'i', $sid);
    if ((int)($inuse['c'] ?? 0) > 0) {
      $flash = 'Cannot delete: subscription has bookings.';
    } else {
      $stmt = mysqli_prepare($conn,"DELETE FROM Subscription WHERE Subscription_ID = ? AND Stable_ID = ?");
      mysqli_stmt_bind_param($stmt,'ii',$sid,$STABLE_ID);
      mysqli_stmt_execute($stmt);
      mysqli_stmt_close($stmt);
      header('Location: sub.php?deleted=1'); exit;
    }
  }
}
if (isset($_GET['added']))   $flash = 'Subscription added successfully.';
if (isset($_GET['deleted'])) $flash = 'Subscription deleted successfully.';

/***** 6) الفلاتر *****/
$filter_name  = trim($_GET['name']  ?? '');
$filter_class = trim($_GET['class'] ?? '');
$filter_days  = trim($_GET['days']  ?? '');

/***** 7) خيارات Class_Type ديناميكيًا للإسطبل *****/
$class_options = fetch_all($conn,"
  SELECT DISTINCT Class_Type AS val
  FROM Subscription
  WHERE Stable_ID = ? AND Class_Type <> ''
  ORDER BY val ASC
",'i',$STABLE_ID);

/***** 8) الاستعلام الرئيسي (اشتراكات هذا الإسطبل فقط) *****/
$sql = "
  SELECT Subscription_ID, Sub_name, SessionLimit, Class_Type, DurationDays, Price
  FROM Subscription
  WHERE Stable_ID = ?
";
$types = 'i';
$params = [$STABLE_ID];

if ($filter_name !== '') {
  $sql .= " AND Sub_name LIKE ? ";
  $types .= 's'; $params[] = '%'.$filter_name.'%';
}
if ($filter_class !== '' && strtolower($filter_class) !== 'all') {
  $sql .= " AND Class_Type = ? ";
  $types .= 's'; $params[] = $filter_class;
}
if ($filter_days !== '') {
  $sql .= " AND DurationDays = ? ";
  $types .= 'i'; $params[] = (int)$filter_days;
}

$sql .= " ORDER BY Subscription_ID DESC ";
$rows = fetch_all($conn, $sql, $types, ...$params);
// عداد إشعارات الطلبات الجديدة (pending اليوم) لإسطبلك
// عداد إشعارات الطلبات الجديدة (pending اليوم أو في المستقبل) لإسطبلك
// هذا هو الاستعلام الذي تم تصحيحه ليتضمن التاريخ الحالي أو المستقبلي (>= CURDATE())
$stmtN = mysqli_prepare($conn, "
  SELECT COUNT(*) AS cnt
  FROM Booking b
  JOIN Subscription s ON s.Subscription_ID = b.Subscription_ID
  WHERE s.Stable_ID = ? 
    AND b.Status = 'pending' 
    AND b.Date >= CURDATE()
");
mysqli_stmt_bind_param($stmtN, 'i', $STABLE_ID);
mysqli_stmt_execute($stmtN);
$resN = mysqli_stmt_get_result($stmtN);
$notif_count = 0;
if ($n = mysqli_fetch_assoc($resN)) $notif_count = (int)$n['cnt'];
mysqli_free_result($resN);
mysqli_stmt_close($stmtN);

// (اختياري) آخر 5 طلبات جديدة لعرضها كقائمة منسدلة
$latest = [];
$stmtL = mysqli_prepare($conn, "
  SELECT b.Booking_ID, b.Time, CONCAT(r.Fname,' ',r.Lname) AS rider, b.Class_Type
  FROM Booking b
  JOIN Subscription s ON s.Subscription_ID = b.Subscription_ID
  JOIN Rider r ON r.Rider_ID = b.Rider_ID
  WHERE s.Stable_ID = ? AND b.Status='pending' AND b.Date >= CURDATE()
  ORDER BY b.Booking_ID DESC
  LIMIT 5
");
mysqli_stmt_bind_param($stmtL, 'i', $STABLE_ID);
mysqli_stmt_execute($stmtL);
$resL = mysqli_stmt_get_result($stmtL);
while($row = mysqli_fetch_assoc($resL)) $latest[] = $row;
mysqli_free_result($resL);
mysqli_stmt_close($stmtL);

?>
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <title>StrideHub - Subscriptions</title>
  <link rel="stylesheet" href="style1.css">
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
</head>
<body>

  <!-- الشريط الجانبي -->
  <div class="sidebar" id="sidebar">
    <a href="dashboard.php"><i class="fa fa-home"></i> Dashboard</a>
    <a href="booking.php"><i class="fa fa-calendar"></i> Booking</a>
    <a href="rider1.php"><i class="fa fa-users"></i> Riders</a>
    <a href="trainer3.php"><i class="fa fa-chalkboard-teacher"></i> Trainers</a>
    <a href="sub.php" class="active"><i class="fa fa-tags"></i> Subscriptions</a>
    <a href="res.php"><i class="fa fa-clipboard-list"></i> Reservations </a>
    <a href="report2.php"><i class="fa fa-file-alt"></i> Report</a>
    <div class="sidebar-bottom">
      <a href="admain.php"><i class="fa fa-user"></i> My Account</a>
      <a href="stable_login.php"><i class="fas fa-sign-out-alt"></i> Signout</a>
    </div>
  </div>

  <!-- الهيدر -->
  <div class="header">
    <i class="fas fa-bars menu-btn" onclick="toggleMenu()" style="cursor:pointer; font-size:20px;"></i>
    <h1 class="logo">Sahwa</h1>
    <div class="icons">
      <div></div>

       <!-- أيقونة التنبيهات مع الشارة -->
      <div class="notif">
        <i class="fas fa-bell"></i>
        <?php if ($notif_count > 0): ?>
          <span class="badge"><?php echo $notif_count; ?></span>
        <?php endif; ?>

        <!-- قائمة منسدلة (اختيارية) لآخر 5 حجوزات جديدة -->
        <?php if (!empty($latest)): ?>
          <div class="notif-menu">
            <?php foreach ($latest as $it): ?>
              <div class="notif-item">
                <strong>#<?php echo (int)$it['Booking_ID']; ?></strong>
                — <?php echo h($it['rider']); ?> (<?php echo h($it['Class_Type']); ?> @ <?php echo h($it['Time']); ?>)
              </div>
            <?php endforeach; ?>
            <a class="notif-more" href="booking.php">View all</a>
          </div>
        <?php endif; ?>
      </div>
      <div class="profile"><img src="<?php echo h($__profile_src__); ?>" alt="Profile" width="80" height="80"></div>
    </div>
  </div>

  <!-- المحتوى -->
  <div class="main">
    <h2>Subscriptions</h2>

    <?php if ($flash !== ''): ?>
      <div class="alert" style="margin-bottom:12px; background:#e6ffed; color:#0b7a33; padding:10px 12px; border-radius:10px;">
        <?php echo h($flash); ?>
      </div>
    <?php endif; ?>
 
    <!-- الفلاتر -->
    <form id="filters" method="get" class="search-box">
      <input type="text" name="name" placeholder="Subscription name" value="<?php echo h($filter_name); ?>">

      <select name="class">
        <option value="">Class Type</option>
        <option value="all" <?php echo ($filter_class==='all'?'selected':''); ?>>All</option>
        <?php foreach ($class_options as $c): ?>
          <option value="<?php echo h($c['val']); ?>" <?php echo ($filter_class===$c['val']?'selected':''); ?>>
            <?php echo h($c['val']); ?>
          </option>
        <?php endforeach; ?>
      </select>

      <input type="number" name="days" placeholder="Duration (Days)" value="<?php echo h($filter_days); ?>">

      <button type="submit" class="btn">Filter</button>
      <a href="sub.php" class="btn">Reset</a>
    </form>

    <!-- زر إضافة -->
    <button class="btn" style="margin:10px 0;" onclick="openModal()">+ Add Subscription</button>

    <!-- الجدول -->
    <div class="table-box">
      <table>
        <thead>
          <tr>
            <th>Subscription ID</th>
            <th>Subscription Name</th>
            <th>Session Limit</th>
            <th>Class Type</th>
            <th>Duration Days</th>
            <th>Price</th>
            <th>Delete</th>
          </tr>
        </thead>
        <tbody>
        <?php if (count($rows) === 0): ?>
          <tr><td colspan="7" class="empty">---</td></tr>
        <?php else: foreach ($rows as $r): ?>
          <tr>
            <td><?php echo h($r['Subscription_ID']); ?></td>
            <td><?php echo h($r['Sub_name']); ?></td>
            <td><?php echo h($r['SessionLimit']); ?></td>
            <td><?php echo h($r['Class_Type']); ?></td>
            <td><?php echo h($r['DurationDays']); ?></td>
            <td><?php echo h($r['Price']); ?></td>
            <td>
              <a href="sub.php?delete=<?php echo (int)$r['Subscription_ID']; ?>"
                 onclick="return confirm('Are you sure to delete this subscription?');"
                 title="Delete" style="color:#d9534f;">
                <i class="fa fa-trash"></i>
              </a>
            </td>
          </tr>
        <?php endforeach; endif; ?>
        </tbody>
      </table>
    </div>
  </div>

  <!-- مودال إضافة اشتراك -->
  <div id="addSubModal" class="modal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,.4); align-items:center; justify-content:center;">
    <div class="modal-card" style="background:#fff; border-radius:16px; padding:20px; min-width:460px; box-shadow:0 20px 50px rgba(0,0,0,.15); position:relative;">
      <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:8px;">
        <h3 style="margin:0;">Add Subscription</h3>
        <button onclick="closeModal()" style="background:none;border:none;font-size:18px;cursor:pointer;">✕</button>
      </div>

      <form method="post" class="modal-form">
        <input type="hidden" name="add_sub" value="1">
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">
          <input type="text"   name="sub_name"      placeholder="Subscription name" required>
          <input type="number" name="session_limit" placeholder="Session limit" min="1" required>
          <input type="text"   name="class_type"    placeholder="Class type" required>
          <input type="number" name="duration_days" placeholder="Duration (days)" min="1" required>
          <input type="number" step="0.01" name="price" placeholder="Price (SAR)" min="0" required>
        </div>
        <div style="margin-top:12px; display:flex; justify-content:flex-end; gap:8px;">
          <button type="button" class="btn-secondary" onclick="closeModal()">Cancel</button>
          <button type="submit" class="btn-primary">Add Subscription</button>
        </div>
      </form>
    </div>
  </div>

<script>
  function toggleMenu() {
    let sidebar = document.getElementById("sidebar");
    let header = document.querySelector(".header");
    let content = document.querySelector(".main");
    if (sidebar.style.left === "-220px") {
      sidebar.style.left = "0";
      header.style.marginLeft = "220px";
      content.style.marginLeft = "220px";
    } else {
      sidebar.style.left = "-220px";
      header.style.marginLeft = "0";
      content.style.marginLeft = "0";
    }
  }
  // أوتو-سبميت للفلاتر
  document.getElementById('filters').addEventListener('change', function(){ this.submit(); });
  // فتح/إغلاق مودال الإضافة
  function openModal(){ document.getElementById('addSubModal').style.display='flex'; }
  function closeModal(){ document.getElementById('addSubModal').style.display='none'; }
</script>
</body>
</html>
