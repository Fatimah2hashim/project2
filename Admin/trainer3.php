...
/***** 4) إضافة مدرب (AUTO_INCREMENT + Stable_ID الحالي) *****/
$flash = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_trainer'])) {
  ...
  if ($Fname === '' || $Lname === '' || $Specialization === '' || $Gender === '' || $Availability === '') {
    $flash = 'Please fill all fields.';
  } else {
    // منع تكرار نفس الاسم داخل نفس الإسطبل (اختياري)
    $exists = fetch_one($conn, "
      SELECT COUNT(*) AS c FROM Trainer 
      WHERE Fname = ? AND Lname = ? AND Stable_ID = ?
    ", 'ssi', $Fname, $Lname, $STABLE_ID);

    if ((int)($exists['c'] ?? 0) > 0) {
      $flash = 'Trainer already exists in your stable.';
    } else {
      // إدخال دون تحديد Trainer_ID (auto_increment)
      $stmt = mysqli_prepare($conn, "
        INSERT INTO Trainer (Fname, Lname, Specialization, Gender, Availability, Stable_ID)
        VALUES (?,?,?,?,?,?)
      ");
      mysqli_stmt_bind_param($stmt, 'sssssi',
        $Fname, $Lname, $Specialization, $Gender, $Availability, $STABLE_ID
      );
      $ok = mysqli_stmt_execute($stmt);
      $err = mysqli_error($conn); // للتشخيص لو صار خطأ
      mysqli_stmt_close($stmt);

      if ($ok) {
        header('Location: trainer3.php?added=1');
        exit;
      } else {
        $flash = 'Insert failed: '.$err;
      }
    }
  }
}

/***** 5) حذف مدرب (ممنوع إذا لديه حجوزات في هذا الإسطبل) *****/
if (isset($_GET['delete']) && ctype_digit($_GET['delete'])) {
  $tid = (int) $_GET['delete'];

  // تحقّق أن المدرب يخص هذا الإسطبل
  $owner = fetch_one($conn, "SELECT Stable_ID FROM Trainer WHERE Trainer_ID = ?", 'i', $tid);
  if (!$owner || (int)$owner['Stable_ID'] !== $STABLE_ID) {
    $flash = 'You can only delete trainers in your stable.';
  } else {
    ...
    $chk = fetch_one($conn,"
      ...
      WHERE b.Trainer_ID = ? AND s.Stable_ID = ?
    ",'ii',$tid,$STABLE_ID);

    if ((int)($chk['c'] ?? 0) > 0) {
      $flash = 'Cannot delete: trainer has bookings in your stable.';
    } else {
      $stmt = mysqli_prepare($conn,"DELETE FROM Trainer WHERE Trainer_ID = ? AND Stable_ID = ?");
      mysqli_stmt_bind_param($stmt,'ii',$tid,$STABLE_ID);
      mysqli_stmt_execute($stmt);
      mysqli_stmt_close($stmt);
      header('Location: trainer3.php?deleted=1');
      exit;
    }
  }
}
if (isset($_GET['added']))   $flash = 'Trainer added successfully.';
if (isset($_GET['deleted'])) $flash = 'Trainer deleted successfully.';

/***** 6) فلاتر القائمة *****/
$filter_name  = trim($_GET['name']  ?? '');
$filter_gender= trim($_GET['gender']?? '');
$filter_spec  = trim($_GET['spec']  ?? '');

/***** 7) خيارات الفلاتر (من مدربي نفس الإسطبل) *****/
...
$spec_options = fetch_all($conn,"
  SELECT DISTINCT Specialization AS val 
  FROM Trainer 
  WHERE Stable_ID = ? AND Specialization <> '' 
  ORDER BY val ASC
",'i',$STABLE_ID);

/***** 8) الاستعلام الرئيسي (مدربي هذا الإسطبل فقط) *****/
$sql = "
  SELECT Trainer_ID, Fname, Lname, Specialization, Gender, Availability
  FROM Trainer
  WHERE Stable_ID = ?
";
$types = 'i';
...
if ($filter_spec !== '' && strtolower($filter_spec) !== 'all') {
  $sql .= " AND Specialization = ? ";
  $types .= 's'; $params[] = $filter_spec;
}
...
<head>
  <meta charset="UTF-8">
  <title>StrideHub - Trainers</title>
...
<body>
  ...
  <div class="sidebar" id="sidebar">
    ...
    <a href="trainer3.php" class="active"><i class="fa fa-chalkboard-teacher"></i> Trainers</a>
  ...
  <div class="main">
    ...
    <form id="filters" method="get" class="search-box">
      <input type="text" name="name" placeholder="Trainer name" value="<?php echo h($filter_name); ?>">
      <select name="gender">
      ...
      <select name="spec">
        ...
        <option value="all" <?php echo ($filter_spec==='all'?'selected':''); ?>>All</option>
        <?php foreach ($spec_options as $sp): ?>
          <option value="<?php echo h($sp['val']); ?>" <?php echo ($filter_spec===$sp['val']?'selected':''); ?>>
            <?php echo h($sp['val']); ?>
          </option>
        <?php endforeach; ?>
      </select>
      <button type="submit" class="btn">Filter</button>
      <a href="trainer3.php" class="btn">Reset</a>
    </form>

    <!-- زر فتح مودال الإضافة -->
    <button class="btn" style="margin:10px 0;" onclick="openModal()">+ Add Trainer</button>
        ...
        <tbody>
          ...
          <?php else: ?>
            <?php foreach ($rows as $r): ?>
              <tr>
                <td><?php echo h($r['Trainer_ID']); ?></td>
                <td><?php echo h($r['Fname']); ?></td>
                <td><?php echo h($r['Lname']); ?></td>
                <td><?php echo h($r['Specialization']); ?></td>
                <td><?php echo h($r['Gender']); ?></td>
                <td><?php echo h($r['Availability']); ?></td>
                <td>
                  <a href="trainer3.php?delete=<?php echo (int)$r['Trainer_ID']; ?>"
  ...
  <!-- مودال إضافة مدرب (بدون حقل Trainer ID) -->
  <div id="addTrainerModal" class="modal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,.4); align-items:center; justify-content:center;">
    <div class="modal-card" style="background:#fff; border-radius:16px; padding:20px; min-width:380px; box-shadow:0 20px 50px rgba(0,0,0,.15); position:relative;">
...
<script>
  ...
  // فتح/إغلاق المودال
  function openModal(){ document.getElementById('addTrainerModal').style.display='flex'; }
  function closeModal(){ document.getElementById('addTrainerModal').style.display='none'; }
</script>
</body>
</html>
